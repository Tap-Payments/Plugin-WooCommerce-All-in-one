...
if ($this->ui_mode == 'tokenization'){
		...
		//	add_filter('woocommerce_order_needs_payment',array($this, 'my_payments'));

           // add_action( 'woocommerce_payment_token_deleted',array('action_woocommerce_payment_token_deleted'), 10, 2 ); 
...
public function webhook($order_id) {
            ...
            $status = $data['status'];
            $charge_id = $data['id'];
           // $pathToFile = 'wp-content/plugins/tap-payments/request.log';
           // file_put_contents($pathToFile, json_encode($data), FILE_APPEND);
            if ($status == 'CAPTURED') {
...
public function tap_thank_you_page($order_id) {
	...
	if ($this->ui_mode == 'popup' || $this->ui_mode == 'redirect' || $this->ui_mode == 'tokenization' ){
            ...
            $Hash = 'x_publickey'. $active_pk.'x_amount'.$Total_price.'x_currency'.$order->currency.'x_transaction'.$ref.'x_post'.$this->post_url;
            $hashstring = hash_hmac('sha256', $Hash, $active_sk);
 		    //  echo  $Hash;exit;
 			// echo '<form id="tap-payment-form">';
 			...
 			//echo '<input type="hidden" id="live_public_key" value="' . $this->live_public_key . '" />';
